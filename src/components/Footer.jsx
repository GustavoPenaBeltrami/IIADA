    ...
    <div className="bg-[#005FAC] min-h-[250px] mt-10 w-full">
      <div className=" grid lg:grid-cols-3 grid-cols-2 w-[90%] mx-auto max-w-[1200px] pt-10 gap-y-10">
        ...
        <div className="flex flex-col gap-y-4 justify-center items-center">
          {links.map((link,index) => (
            <Link
              key={index}
              href={link.href}
              className={`text-white text-md mt-2 font-bold hover:underline ${link.href === "/login" ? "hidden" : "block"}`}
            >
              {link.label}
            </Link>)
        ...
        <div className="lg:col-span-1 col-span-2 mb-10 lg:mb-0">
          <Image src="/Naylor.webp" alt="Hero Image" width={300} height={200} className="mx-auto" objectFit="cover"/>
        </div>
      </div>
    </div>
  );
